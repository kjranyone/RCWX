...
from rcwx.pipeline.f0_processing import (  # noqa: F401  (shared + re-exported)
    F0_CORRECTION_SUSTAIN_FRAMES,
    MAX_MOE_BOOST,
    _align_f0_frames,
    _smooth_fcpe_f0,
    align_f0_to_feature_grid,
...
class RVCPipeline:
    ...
    # ------------------------------------------------------------------
    # Shared pipeline stages (used identically by infer / infer_streaming)
    # ------------------------------------------------------------------

    def _hubert_output_params(self) -> tuple[int, int]:
        """(output_layer, output_dim) for the loaded model version.
    ...
    def _extract_hubert_features(
        ...
        output_layer: int,
        output_dim: int,
        prof: Optional[_StageProfiler] = None,
    ) -> torch.Tensor:
        """Run HuBERT feature extraction under autocast (optionally profiled)."""
    ...
    def _extract_f0_raw(
        self,
        f0_input: torch.Tensor,
        f0_method: str,
        prof: Optional[_StageProfiler] = None,
    ) -> Optional[torch.Tensor]:
        """Dispatch F0 extraction to the selected backend (RMVPE fallback).

        SwiftF0 runs synchronously on CPU — wall time is exact there; the GPU
        methods use device timing events when a profiler is provided.
        ...
        if f0_method == "fcpe" and self.fcpe is not None:
            with torch.autocast(device_type=self.device, dtype=self.dtype):
                result = self.fcpe.infer(f0_input, threshold=FCPE_VOICING_THRESHOLD)
        elif f0_method == "swiftf0" and self.swiftf0 is not None:
            result = self.swiftf0.infer(f0_input, threshold=SWIFTF0_VOICING_THRESHOLD)
        elif self.rmvpe is not None:
            with torch.autocast(device_type=self.device, dtype=self.dtype):
                result = self.rmvpe.infer(f0_input, threshold=RMVPE_VOICING_THRESHOLD)
        else:
            logger.warning(
                "[INFER] F0 extraction returned None "
    ...
    def _condition_raw_f0(
        self,
        f0: torch.Tensor,
        f0_method: str,
        residual_f0_shift: float,
        moe_strength: float,
    ) -> torch.Tensor:
        """Shared raw-F0 conditioning: FCPE smoothing -> pitch shift -> moe."""
        if f0_method == "fcpe":
        ...
        if abs(residual_f0_shift) > 0.01:
            f0 = torch.where(f0 > 0, f0 * (2 ** (residual_f0_shift / 12)), f0)
        if moe_strength > 0.0:
            f0 = apply_moe_f0_style(f0, moe_strength)
        return f0

    def _flat_pitch_fallback(
        self, features: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Unvoiced pitch=1 / pitchf=0 tracks matching ``features`` (RVC convention)."""
        pitch = torch.ones(
    ...
    def _interpolate_features_2x(self, features: torch.Tensor) -> torch.Tensor:
        """Interpolate HuBERT features 50fps -> 100fps for the synthesizer.

        Linear interpolation is perceptually smoother and helps reduce
        metallic buzz (fixed 2x upscale, matches original RVC).
        """
    ...
    def _pad_features_to_min(
        ...
        """Reflect/replicate-pad features (+pitch tracks) up to ``min_frames``.

        The synthesizer decoder's upsampling convolutions require a minimum
        input length. Returns the (possibly padded) tensors and the number of
        frames added — 0 when already long enough; the caller refreshes
        feature_lengths only when frames were added.
        """
        synth_pad_frames = min_frames - features.shape[1]
        if synth_pad_frames <= 0:
            return features, pitch, pitchf, 0
        pad_left = synth_pad_frames // 2
        pad_right = synth_pad_frames - pad_left

        # reflect mode requires padding < input size; use replicate for very
        # short inputs
        ...
        logger.info(
            f"Padded short features: {current_size} -> {features.shape[1]} frames (min={min_frames})"
        )
        return features, pitch, pitchf, synth_pad_frames
    ...
    def infer(
        ...
        output_layer, output_dim = self._hubert_output_params()
        ...
        if use_parallel_extraction and use_f0 and not graph_capture_pending:
            ...
            f0_future = executor.submit(self._extract_f0_raw, f0_audio, f0_method)
            features = self._extract_hubert_features(audio, output_layer, output_dim)
        ...
        # Interpolate features to match synthesizer expectation (50fps -> 100fps)
        original_frames = features.shape[1]
        features = self._interpolate_features_2x(features)
        logger.info(
        ...
        if use_f0:
            ...
            if f0 is None:
                f0 = self._extract_f0_raw(f0_audio, f0_method)
                logger.debug(f"F0 extracted sequentially ({f0_method})")
            else:
            ...
            if f0 is not None and f0.numel() > 0:  # Check for non-empty F0
                # Shared conditioning: FCPE smoothing -> pitch shift -> moe
                f0 = self._condition_raw_f0(
                    f0, f0_method, residual_f0_shift, moe_strength
                )

                # Align F0 length with features (deficit-at-right convention).
                f0 = align_f0_to_feature_grid(f0, features.shape[1])
            ...
            if f0 is None:
                ...
                pitch, pitchf = self._flat_pitch_fallback(features)
        ...
        features, pitch, pitchf, synth_pad_frames = self._pad_features_to_min(
            features, pitch, pitchf, min_synth_frames
        )
        if synth_pad_frames > 0:
    ...
    def infer_streaming(
        ...
        **overrides,
        ...
        if use_parallel_extraction and use_f0 and not graph_capture_pending:
            executor = self._get_parallel_executor()
            f0_future = executor.submit(
                self._extract_f0_raw, f0_input, f0_method, prof
            )
            features = self._extract_hubert_features(
                audio_t, output_layer, output_dim, prof
            )
            f0_raw = f0_future.result()
        else:
            features = self._extract_hubert_features(
        ...
        if use_f0:
            ...
            if f0 is None:
                ...
                f0 = self._extract_f0_raw(f0_input, f0_method, prof)
            ...
            if f0 is not None and f0.numel() > 0:
                ...
                if f0_window_samples > 0:
                    ...
                    # Same deficit-at-right convention as the batch path.
                    f0 = align_f0_to_feature_grid(f0, features.shape[1])
        ...
        features, pitch, pitchf, synth_pad_frames = self._pad_features_to_min(
            features, pitch, pitchf, MIN_SYNTH_FEATURE_FRAMES
        )
        if synth_pad_frames > 0:
            feature_lengths = torch.tensor(
                [features.shape[1]], dtype=torch.long, device=self.device,
