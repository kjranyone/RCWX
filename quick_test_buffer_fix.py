"""Quick test of SOLA buffer position fix."""
...
def load_audio(path: str, max_sec: float = 1.0) -> np.ndarray:
...
def main():
    print("Quick test of SOLA buffer position fix")

    pipeline = RVCPipeline("sample_data/hogaraka/hogarakav2.pth", use_compile=False)
    pipeline.load()
    audio = load_audio("sample_data/seki.wav", max_sec=1.0)
    ...
    print()
    print("Expected: ~20-25 for 1s audio (proportional to 115 for 5s)")
    print("If significantly lower → fix is working!")
